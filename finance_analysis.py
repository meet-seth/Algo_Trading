...
# This Class analyses the financial instruments example: stocks etc. Any stock
...
class FinancialInstrument:
    ...
    :param ticker: The ticker of the stock whose price shall be downloaded. e.g. for Infosys India it would be "INFY.NS"
                   where INFY is the name of the stock and NS is for NSE stock market.
    ...
    def __init__(self, ticker: str, interval: str, start: str = None, end: str = None, period: str = None):
        """

        :param ticker: The ticker of the stock whose price shall be downloaded. e.g. for Infosys India it would be
                       "INFY.NS" where INFY is the name of the stock and NS is for NSE stock market.
        :param start: The start date from which the stock prices shall be downloaded in format "YYYY-MM-DD"
        :param end: The end date till which the stock prices shall be downloaded in format "YYYY-MM-DD"
    ...
    def __repr__(self):
        if self.period:
            s = f"FinancialInstrument(ticker={self._ticker}, period:{self.period})"
        else:
            s = f"FinancialInstrument(ticker={self._ticker}, start={self.start},end={self.end})"
        return s
    ...
    def get_data(self):
        ...
        """
        if self.period:
            self.data = yf.download(self._ticker, period=self.period, interval=self.interval)
        else:
            self.data = yf.download(self._ticker, self.start, self.end, interval=self.interval)

        # self.data.rename(columns={'Close': 'price'},inplace = True)

    def log_returns(self, horizon=1):
        """
        :param horizon: Calculates returns from horizon
        Adds a column log_returns to "self.data" which contains the daily log_returns of the
        stock price.
        """
        self.data["log_returns"] = np.log(self.data.Close / self.data.Close.shift(horizon))
    ...
    def std_returns(self, freq=None):
        ...
        :param freq: the frequency for a stock price if it is None then the standard deviation of all prices is
                     calculated else it is calculated for a stipulated freq value
